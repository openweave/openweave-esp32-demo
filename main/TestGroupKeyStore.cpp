...
void DumpGroupKeys()
{
    ...
    err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->EnumerateGroupKeys(WeaveKeyId::kType_None, keyIds, kKeyIdListSize, keyCount);
    ...
    {
        err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->RetrieveGroupKey(keyIds[i], key);
        if (err != WEAVE_NO_ERROR)
        {
...
void TestGroupKeyStore()
{
    ...
    err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->StoreGroupKey(key);
    if (err != WEAVE_NO_ERROR)
    {
    ...
    err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->DeleteGroupKey(WeaveKeyId::MakeAppGroupMasterKeyId(0x24));
    if (err != WEAVE_NO_ERROR)
    {
        ESP_LOGE(TAG, "StoreGroupKey() failed: %s", ErrorStr(err));
        return;
    ...
    err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->DeleteGroupKeysOfAType(WeaveKeyId::kType_AppEpochKey);
    if (err != WEAVE_NO_ERROR)
    {
        ESP_LOGE(TAG, "StoreGroupKey() failed: %s", ErrorStr(err));
        return;
    ...
    err = ::nl::Weave::DeviceLayer::FabricState.GroupKeyStore->Clear();
    if (err != WEAVE_NO_ERROR)
    {
        ESP_LOGE(TAG, "StoreGroupKey() failed: %s", ErrorStr(err));
        return;
